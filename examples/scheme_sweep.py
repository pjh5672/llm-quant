...
Every run goes through llmquant.entrypoints.run, the same code path examples/auto_llm.py
...
from llmquant.analysis import format_report, shortlist, summarize
from llmquant.args.parser import build_config, expand_sweep
from llmquant.entrypoints.run import run_generation, run_one
...
def main():
    ...
    print()
    print(f"wrote {out}")
    print(f"re-analyse later with: python examples/analyze_sweep.py {out}")
